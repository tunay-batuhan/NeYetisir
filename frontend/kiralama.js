// Anonim "tarlanı kiraya ver" formu → POST /api/kiralik-basvuru
(() => {
  const $ = (id) => document.getElementById(id);
  const form = $("kiraya-form");
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const body = {
      ...
      ada: $("f-ada").value.trim(),
      parsel: $("f-parsel").value.trim(),
      alan_m2: alanStr === "" ? null : Number(alanStr),
      egim: opt($("f-egim").value),
      su_durumu: opt($("f-su_durumu").value),
    ...
    btn.disabled = true;
    try {
      const r = await fetch("/api/kiralik-basvuru", {
        method: "POST",
